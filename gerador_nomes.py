...
def generate_name(model, start_seq, char_to_int, int_to_char, seq_length, min_length=5, max_length=12, temperature=1.0):
    name = start_seq
...
def incremental_training(model, generated_names, char_to_int, seq_length=5, batch_size=64, epochs=1):
    ...
    #print("Treinamento incremental completo com os nomes gerados.")

# Função para gerar um novo nome e retornar
def gerar_e_retornar_nome():
...
# Código principal para carregar ou treinar o modelo
escolha = "m" #input("Você quer gerar um nome masculino ou feminino? (m/f): ").lower()
file_path = 'nomes_masculinos.txt' if escolha == 'm' else 'nomes_femininos.txt'
X, y, char_to_int, int_to_char, chars, names = load_names(file_path)

model_filename = 'nome_modelo.keras'
